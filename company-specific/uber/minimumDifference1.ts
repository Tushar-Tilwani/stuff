...
function minimumDifference(nums: number[]): number {
  ...
  const minPrefix: number[] = new Array(nums.length).fill(Infinity);
  const maxHeap = new CustomHeap<number>((a, b) => b - a);
  let minSum = 0;
  for (let i = 0; i < nums.length; i++) {
    minSum += nums[i];
    maxHeap.push(nums[i]);
    if (maxHeap.size() > partLength) {
      minSum -= maxHeap.pop()!;
    }
    if (maxHeap.size() === partLength) minPrefix[i] = minSum;
  }

  const maxPrefix: number[] = new Array(nums.length).fill(-Infinity);
  const minHeap = new CustomHeap<number>((a, b) => a - b);
  let maxSum = 0;
  for (let i = nums.length - 1; i >= 0; i--) {
    maxSum += nums[i];
    minHeap.push(nums[i]);
    if (minHeap.size() > partLength) {
      maxSum -= minHeap.pop()!;
    }
    if (minHeap.size() === partLength) maxPrefix[i] = maxSum;
  }

  const result = [Infinity];

  for (let i = partLength; i <= 2 * partLength; i++) {
    result[0] = Math.min(result[0], minPrefix[i] - maxPrefix[i + 1]);
  }

  //   console.log(minPrefix, maxPrefix);

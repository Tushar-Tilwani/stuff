...
function findZeroSum(arr) {
  ...
  const indexMap = arr.reduce((acc, value, index) => {
    if (!acc.has(value)) {
      ...
      acc.set(value, [...acc.get(value), index]);
    }
  ...
  for (let i = 0; i < len; i++) {
    ...
    for (let j = 0; j < len; j++) {
      ...
      const indices = indexMap.get(complememnt);
      indices.forEach(k => {
        if (k !== j && k !== i) {
          result.add([innerValue, complememnt, outerValue].sort().toString());
        }
      });
    }
  }
  return Array.from(result.values()).map(uniqueArr =>
    uniqueArr.split(",").map(val => parseInt(val))
  );
}
